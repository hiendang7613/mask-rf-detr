...
def main():
    ...
    parser.add_argument(
        "--image", "-i", type=str, help="Path to a single image for inference"
    )
    parser.add_argument(
        "--input_dir", type=str, help="Path to directory containing images"
    )
    parser.add_argument(
        "--output_dir",
        "-o",
    ...
    for img_path in image_paths:
        ...
        for i in range(len(detections)):
            ...
            if config["class_names"] and detections.class_id[i] < len(
                config["class_names"]
            ):
