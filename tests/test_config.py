...
pytestmark = pytest.mark.skipif(
    not HAS_TORCH or not HAS_PYDANTIC,
    reason="Requires torch and pydantic"
)
...
class TestModelConfig:
    ...
    def test_model_config_defaults(self):
        ...
        assert config.two_stage == True
        assert config.bbox_reparam == True
        assert config.lite_refpoint_refine == True
        assert config.layer_norm == True
        assert config.amp == True
        assert config.num_classes == 90
        assert config.resolution == 560
        assert config.group_detr == 9
        assert config.gradient_checkpointing == False
...
class TestTrainConfig:
    ...
    def test_train_config_defaults(self):
        ...
        assert config.ia_bce_loss == True
        assert config.cls_loss_coef == 1.0
        assert config.num_select == 300
        assert config.dataset_file == "roboflow"
        assert config.square_resize_div_64 == True
        assert config.output_dir == "output"
        assert config.multi_scale == True
        assert config.expanded_scales == True
        ...
        assert config.early_stopping_use_ema == False
        assert config.tensorboard == True
        assert config.wandb == False
        assert config.run_test == True
