...
class Model:
    def __init__(self, **kwargs):
        ...
        if args.pretrain_weights is not None:
            ...
            new_state_dict = {
                k.replace("module.", ""): v for k, v in new_state_dict.items()
            }
            ...
            for k, v in new_state_dict.items():
                if k not in current_model_dict:
                    absent_in_model.append(k)
                elif v.size() != current_model_dict[k].size():
                    shape_mismatch.append(
                        (k, tuple(v.size()), tuple(current_model_dict[k].size()))
                    )
                else:
            ...
            missing, unexpected = self.model.load_state_dict(
                current_model_dict, strict=False
            )
            logger.info(
                f"Pretrain weights: loaded {len(compatible_state_dict)}/{len(new_state_dict)} keys "
                f"(shape-mismatch {len(shape_mismatch)}, absent-in-model {len(absent_in_model)}, "
                f"randomly-initialized {len(missing)})"
            )
            if shape_mismatch:
                logger.warning(f"Shape mismatches (first 10): {shape_mismatch[:10]}")
            if missing:
                logger.warning(
                    f"Randomly-initialized params (first 10): {missing[:10]}"
                )
    ...
    def train(self, callbacks: DefaultDict[str, List[Callable]], **kwargs):
        ...
        if mask_weights_path and os.path.exists(mask_weights_path):
            ...
            logger.info(
                f"Mask weights: loaded {len(compatible_state_dict)}/{len(new_state_dict)} keys "
                f"(shape-mismatch {len(shape_mismatch)}, absent-in-model {len(absent_in_model)})"
            )
            if shape_mismatch:
                logger.warning(f"Shape mismatches (first 10): {shape_mismatch[:10]}")
    ...
    def export(
        ...
        **kwargs,
        ...
        if backbone_only:
            ...
            # forward_export returns (boxes, logits[, masks]) — match that order.
